...
import { storeTraceFinderWorkflowResult } from '../lib/trace-finder-workflow'
...
function fixture() {
  ...
  const instance = Object.assign(new WorkflowInstance(), {
    id: randomUUID(), workflowId: 'photographers.hidden_potential', currentStepId: 'o2',
  ...
  const run = {
    ...
    input: { registrationId: registration.id, firstName: registration.firstName, lastName: registration.lastName, email: registration.email, portfolioRaw: '' },
    output: { kind: 'research', data: { status: 'no_results', candidates: [], summary: 'Brak trafień.', issues: [] } },
  }
...
test('stores scoped partial discovery through the command and returns references only', async () => {
  const setup = fixture()
  await expect(storeTraceFinderWorkflowResult({ runId: setup.run.id }, setup.context, setup.container)).resolves.toEqual({ runId: setup.run.id, tracesRef: setup.materialId, status: 'no_results' })
  ...
  expect(findOneWithDecryption).toHaveBeenCalledWith(setup.em, AgentRun, expect.objectContaining({ id: setup.run.id, workflowInstanceId: setup.instance.id, stepId: 'o2', agentId: 'photographers.trace_finder', status: 'ok', resultKind: 'research' }), {}, setup.scope)
  expect(findOneWithDecryption).toHaveBeenCalledWith(setup.em, StepInstance, expect.objectContaining({ id: setup.run.invocationId, workflowInstanceId: setup.instance.id, stepId: 'o2' }), {}, setup.scope)
})

test.each(['registrationId', 'firstName', 'lastName', 'email', 'portfolioRaw'] as const)('rejects a mismatched run input %s before storing', async (field) => {
  const setup = fixture()
  setup.run.input[field] = field === 'registrationId' ? randomUUID() : field === 'email' ? 'someone@example.invalid' : 'different'
  await expect(storeTraceFinderWorkflowResult({ runId: setup.run.id }, setup.context, setup.container)).rejects.toThrow('does not belong')
...
test.each([WorkflowInstance, AgentRun, StepInstance, PhotographerRawData])('rejects missing or cross-scoped records', async (entity) => {
  ...
  await expect(storeTraceFinderWorkflowResult({ runId: setup.run.id }, setup.context, setup.container)).rejects.toThrow()
  expect(setup.execute).not.toHaveBeenCalled()
})
...
test('requires permissions before accessing research', async () => {
  ...
  await expect(storeTraceFinderWorkflowResult({ runId: setup.run.id }, setup.context, setup.container)).rejects.toThrow('access denied')
...
test.each([0, 2])('rejects ambiguous step history with %s attempts', async (attempts) => {
  ...
  await expect(storeTraceFinderWorkflowResult({ runId: setup.run.id }, setup.context, setup.container)).rejects.toThrow('attempt does not match')
  expect(setup.execute).not.toHaveBeenCalled()
})
...
test('rejects an incomplete run', async () => {
  ...
  await expect(storeTraceFinderWorkflowResult({ runId: setup.run.id }, setup.context, setup.container)).rejects.toThrow('not complete')
  expect(setup.execute).not.toHaveBeenCalled()
})

test.each(['identity', 'end'])('cannot attach output after workflow leaves O2 for %s', async (stepId) => {
  const setup = fixture()
  setup.instance.currentStepId = stepId
  await expect(storeTraceFinderWorkflowResult({ runId: setup.run.id }, setup.context, setup.container)).rejects.toThrow('not current')
  expect(setup.execute).not.toHaveBeenCalled()
})
