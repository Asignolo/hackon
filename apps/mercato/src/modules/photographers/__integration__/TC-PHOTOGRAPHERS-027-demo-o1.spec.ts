...
test.describe('TC-PHOTOGRAPHERS-027: demo runs O1 and waits for O2', () => {
  ...
  for (const hasPortfolio of [true, false]) {
    test(hasPortfolio ? 'portfolio registration persists sourced traces' : 'registration without portfolio still invokes O1 and persists email discovery', async ({ request }) => {
      ...
      const fixture = await registrationCrmFixture(request, ['workflows.*', 'agent_orchestrator.*', 'configs.manage'])
      ...
      try {
        ...
        await expect.poll(async () => {
          ...
          return withClient(async (db) => {
            ...
            return withClient(async (db) => (await db.query<{ current_step_id: string }>('select current_step_id from workflow_instances where id=$1', [instance.id])).rows[0]?.current_step_id)
          })
        }, { timeout: 45_000 }).toBe('await_o2_integration')
        ...
        expect(await readJsonSafe(finalResponse)).toMatchObject({ status: 'running', proposalId: null, o1: { status: 'completed', tracesRef: rows[0].id, nextStage: 'o2', sourcesAccepted: true } })
