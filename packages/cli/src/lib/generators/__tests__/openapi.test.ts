...
`
  ...
  it('bundles package JSON dependencies and refreshes their request and response schemas', async () => {
    const schemaPath = path.join(tmpDir, 'node_modules', 'schema-fixture', 'schemas.json')
    touchFile(
      path.join(tmpDir, 'node_modules', 'schema-fixture', 'package.json'),
      JSON.stringify({ name: 'schema-fixture', version: '1.0.0' }),
    )
    const schemas = {
      request: { type: 'object', properties: { name: { type: 'string' } }, required: ['name'] },
      response: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
    }
    touchFile(schemaPath, JSON.stringify(schemas))
    touchFile(
      path.join(tmpDir, 'packages', 'core', 'src', 'modules', 'demo', 'api', 'items', 'route.ts'),
      [
        "import schemas from 'schema-fixture/schemas.json'",
        'export async function POST() { return new Response("ok") }',
        'export const openApi = { POST: {',
        '  requestBody: { content: { "application/json": { schema: schemas.request } } },',
        '  responses: { "201": { description: "Created", content: { "application/json": { schema: schemas.response } } } },',
        '} }',
      ].join('\n'),
    )
    const resolver = createMockResolver([{ id: 'demo', from: '@open-mercato/core' }])
    ...
    const operation = doc.paths['/api/demo/items'].post
    expect(operation.requestBody.content['application/json'].schema).toEqual(schemas.request)
    expect(operation.responses['201'].content['application/json'].schema).toEqual(schemas.response)

    const updatedRequest = { ...schemas.request, required: [] }
    touchFile(schemaPath, JSON.stringify({ ...schemas, request: updatedRequest }))
    const updatedResult = await generateOpenApi({ resolver, quiet: true })
    expect(updatedResult.filesWritten).toEqual([generatedPath])
    const updatedDoc = JSON.parse(fs.readFileSync(generatedPath, 'utf8'))
    expect(updatedDoc.paths['/api/demo/items'].post.requestBody.content['application/json'].schema)
