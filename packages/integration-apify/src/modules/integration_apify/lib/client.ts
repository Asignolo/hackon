...
function createClient(
  ...
  maxRetries: number,
  timeoutSecs: number,
  responseLimit?: typeof applyApifyReadResponseLimit,
): ApifyClientLike {
  ...
  return new ApifyClient({
    ...
    maxRetries,
    timeoutSecs,
    requestInterceptors: responseLimit ? [responseLimit] : [],
...
export function createApifyReadClient(
  ...
  return createClient(credentials, 2, timeoutSecs, applyApifyReadResponseLimit)
}

export function createApifyHealthClient(
  credentials: Record<string, unknown>,
): ApifyClientLike {
  return createClient(credentials, 0, 3, applyApifyReadResponseLimit)
}
