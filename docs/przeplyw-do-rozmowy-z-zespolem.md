...
Open Mercato Agent Orchestrator prowadzi ocenę jednego fotografa. Uruchamia zadania, zbiera wyniki, zatrzymuje pracę przed wymaganą decyzją człowieka i wykonuje zatwierdzone zmiany. Odkrycie składa się z dwóch zadań dla agentów i osobnego kroku, który sprawdza dowody według reguł.

## 1. Od rejestracji do pewnych śladów

Ślad to adres lub identyfikator miejsca, w którym fotograf występuje: konto społecznościowe, strona, galeria, wizytówka Google Maps lub wpis w rejestrze.
...
flowchart TD
    ...
    O1["O1 · Odczyt portfolio · AI<br/>Rozpoznaj wpis, otwórz portfolio<br/>Zbierz adresy i poszlaki"]
    O2["O2 · Dalsze poszukiwania · AI<br/>Własna strona i stopka → e-mail w wyszukiwarce<br/>→ kandydaci w rejestrach"]
    K1["K1 · Sprawdzenie tożsamości · kod<br/>Porównaj dowody dla każdego śladu<br/>Nadaj pewność i zapisz uzasadnienie"]
    CLASS{"Wynik dla danego śladu"}
    POLICY["Pewny ślad<br/>Propozycja tożsamości → polityka zatwierdzania"]
    HUMAN["Caseload · operator<br/>Zatwierdź, popraw lub odrzuć"]
    CAND["Nierozstrzygnięty kandydat<br/>Zapisz powód; nie badaj tego śladu"]
    SET["Zbierz zaakceptowane ślady<br/>Pomiń odrzucone i nierozstrzygnięte"]
    ANY{"Czy jest co badać?"}
    RESEARCH["Badanie tylko po pewnych śladach"]
    WAIT["Obserwowana<br/>Brak użytecznych śladów po wyczerpaniu poszukiwań<br/>Kolejna ocena za 180 dni"]
    REG --> START --> O1
    O1 -->|"Poszlaki lub zapis przyczyny ich braku"| O2
    REG -.->|"Imię, nazwisko, e-mail"| O2
    O1 -.->|"Portfolio wskazane przez fotografa"| K1
    O2 -->|"Kandydaci + dowody + źródła"| K1
    K1 --> CLASS
    CLASS -->|"Pewny"| POLICY
    CLASS -->|"Jeden kandydat, jedna poszlaka"| HUMAN
...
Decyzje dotyczą poszczególnych śladów. Niepewny wpis CEIDG nie unieważnia pewnego portfolio. Do badania może trafić sam profil społecznościowy, a fakty rejestrowe pozostaną nieznane. W tej propozycji ocena czeka na rozstrzygnięcie już utworzonych zadań Caseload, zanim zbierze końcowy zestaw śladów.

## 2. Od pewnych śladów do decyzji o kontakcie

```mermaid
flowchart TD
    INPUT["Pewne ślady fotografa"]
...
**Oddaje:** rozpoznane portfolio, znalezione adresy i poszlaki ze źródłami. O1 nie przeszukuje rejestrów ani nie mierzy aktywności konta. Jego zadanie kończy się po odczycie wskazanego portfolio i zebraniu odsyłaczy do dalszego sprawdzenia.
...
3. Wyszukuje pełny e-mail w cudzysłowie. Trafienia w agregatorach zapisuje jako poszlaki.
4. Szuka kandydatów w rejestrach. Korzysta z wcześniejszych ustaleń, żeby zawęzić wyszukiwanie po nazwisku.

**Oddaje:** listę kandydatów i dowodów. Nie wskazuje arbitralnie zwycięzcy spośród kilku osób o tym samym nazwisku. Nie zbiera jeszcze pełnego zestawu faktów do punktacji.
...
**Dostaje:** cztery pola rejestracji i dowody z O1 oraz O2.

Sprawdza każdy ślad osobno według reguł z dokumentacji. Portfolio podane przez fotografa jest mocną kotwicą. Dla wpisu CEIDG lub VAT wymagana jest zgodność nazwiska i co najmniej jednej niezależnej poszlaki wskazanej w §6.4 procesu. Sam NIP z agregatora nie wystarcza. Kilka kopii tej samej informacji nie powinno być traktowane jako niezależne potwierdzenia.

**Oddaje:** pewność, uzasadnienie oraz propozycję tożsamości, jeśli są do niej podstawy. Brak rozstrzygnięcia pozostawia jako brak rozstrzygnięcia. Model może pomóc odczytać i uporządkować treść dowodów; sprawdzenie wymaganych warunków należy do kodu. Nie proponujemy osobnego agenta, który sam ustala reguły pewności.

### A2, A3 i R1. Badanie równoległe

Każdy dostaje tylko pewne ślady ze swojego zakresu oraz potrzebną historię wcześniejszej oceny.
...
| A3 Portfolio | Specjalizację z portfolio, stronę, rezerwacje, system galerii i pewną wizytówkę Google Maps | Kategorię oraz fakty o stronie, galerii i opiniach. Tylko A3 odpowiada za kategorię. |
...
Brak danych zawsze oznacza „nieznane”. Jeśli nie ma pewnego wpisu rejestrowego, R1 kończy z takim wynikiem. Nie rozpoczyna nowego szukania osoby. Nowy kandydat znaleziony podczas badania wraca do odkrycia, zanim stanie się źródłem faktów.
...
- Na fotografie: pewne ślady i fakty wraz ze źródłami.
...
2. **Reguły tożsamości.** Dokumentacja dopuszcza zgodność nazwiska i jednej niezależnej poszlaki, w tym fotograficznego PKD. Czy to wystarcza przy popularnym nazwisku? Diagram zachowuje tę regułę; zespół powinien ją świadomie potwierdzić lub zaostrzyć.
